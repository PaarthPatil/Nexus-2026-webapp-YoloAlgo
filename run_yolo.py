...
model = torch.hub.load('yolov5', 'custom', path='box_detection.pt', source='local')
model.conf = 0.50 # General threshold
model.iou = 0.45
...
# Open the video
cap = cv2.VideoCapture("boxvid.mp4")
os.makedirs("output_folder", exist_ok=True)
fps_in = cap.get(cv2.CAP_PROP_FPS)
width  = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
fourcc = cv2.VideoWriter_fourcc(*'mp4v')
out_video = cv2.VideoWriter("output_folder/tracking_output.mp4", fourcc, fps_in, (width, height))
