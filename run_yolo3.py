...
OUTPUT_MP4 = f"{OUTPUT_DIR}/tracking_output3.mp4"
...
# ==========================================
# 2. MODEL-LEVEL OPTIMIZATIONS (The Smart Math)
# ==========================================
model.conf = CONFIDENCE_THRESHOLD 
model.iou = IOU_THRESHOLD 

# OPTIMIZATION: Class-Agnostic NMS
# If 'True', YOLO will delete overlapping boxes even if they are labelled as entirely different classes!
# This forces the AI to only commit to one physical object per physical space, heavily reducing "messy" tightly packed overlaps.
model.agnostic = True 

# OPTIMIZATION: Max Detections
# YOLO evaluates 1,000 predictions by default per frame. If you know there are never more than 50 boxes on screen, 
# you can cap it to 50. It saves the AI from doing 950 unnecessary "Non-Max Suppression" calculations per frame!
model.max_det = 100 
...
# OPTIMIZATION: Core Filtering
# Instead of Python figuring out if a box is what we want, we tell the Neural Network to completely 
# ignore predicting all other classes! It won't even try to process 'Person' anymore!
model.classes = bigger_box_classes + small_box_classes
...
while cap.isOpened():
    ...
    # We run prediction on the frame natively 
    results = model(frame)
    ...
    for det in detections:
        ...
        # Because we used model.classes earlier, we are mathematically guaranteed that NO 'Person' class will slip through here!
